...
require('react-native'),View=_require.View,Text=_require.Text,TouchableHighlight=_require.TouchableHighlight;

function renderRowWithoutButtons(item){
return React.createElement(View,{key:item.key},item.input);
}

function renderRowButton(button,stylesheet,style){
return(
React.createElement(TouchableHighlight,{key:button.type,style:[stylesheet.button,style],onPress:button.click},
React.createElement(Text,{style:stylesheet.buttonText},button.label)));
...
buttons.map(function(button){return renderRowButton(button,stylesheet,{width:50});})));
...
return(
React.createElement(View,{key:item.key,style:{flexDirection:'row'}},

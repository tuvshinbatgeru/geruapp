...
var _ProjectInitial = require('../ProjectInitial');

var _ProjectInitial2 = babelHelpers.interopRequireDefault(_ProjectInitial);

var initialState = new _ProjectInitial2.default();
...
function project() {
	...
	switch (action.type) {
		...
		case _ProjectConstants.GET_PROJECTS_WITH_SKILLS_FULFILLED:
			{
				...
				var nextState = state.setIn(['filteredProjects', 'data'], data).setIn(['filteredProjects', 'fullData'], fullData).setIn(['filteredProjects', 'pageLast'], pageLast).setIn(['filteredProjects', 'fetching'], false);
				return nextState;
			}
		...
		case _ProjectConstants.SET_SELECTED_PROJECT:
			{
				...
				return state.setIn(['selectedProject', 'index'], index).setIn(['selectedProject', 'project'], project).setIn(['filteredProjects', 'fullData'], fullData);
			}

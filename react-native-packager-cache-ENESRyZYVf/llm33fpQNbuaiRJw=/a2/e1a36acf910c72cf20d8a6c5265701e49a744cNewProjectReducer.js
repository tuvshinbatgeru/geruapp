...
var _NewProjectInitialState = require('../NewProjectInitialState');

var _NewProjectInitialState2 = babelHelpers.interopRequireDefault(_NewProjectInitialState);
...
var _projectFormValidation = require('../projectFormValidation');

var _projectFormValidation2 = babelHelpers.interopRequireDefault(_projectFormValidation);

var initialState = new _NewProjectInitialState2.default();
...
function newProject() {
	...
	switch (action.type) {
		...
		case _ProjectConstants.NEW_PROJECT:
			{
				var _nextState = state.setIn(['form', 'fields', 'title'], '').setIn(['form', 'fields', 'description'], '').setIn(['form', 'fields', 'awardDate'], new Date()).setIn(['form', 'error'], null).setIn(['form', 'isFetching'], false);
		...
		case _ProjectConstants.SUGGESTED_IMAGE_CHOOSED:
			{
				...
				var _nextState2 = state.setIn(['files', 'selectedFiles', 'data'], selectedFiles).setIn(['files', 'selectedFiles', 'count'], count);
				return _nextState2;
			}
		...
		case _ProjectConstants.GET_SUGGESTED_SHOWCASES_FULFILLED:
			{
				...
				var _nextState4 = state.setIn(['files', 'imagesTabs', 1, 'fetching'], false).setIn(['files', 'imagesTabs', 1, 'data'], suggestedData);
		...
		case _ProjectConstants.TAG_DISELECT:
			{
				...
				var _nextState6 = state.setIn(['tags', 'selected'], _tagsData).setIn(['tags', 'suggested'], _suggestedData);
				return _nextState6;
			}
		...
		case _ProjectConstants.TAG_SELECTED:
			{
				...
				var _nextState7 = state.setIn(['tags', 'selected'], _tagsData2).setIn(['tags', 'suggested'], _suggestedData2);
				return _nextState7;
			}
		...
		case _ProjectConstants.GET_TAGS_FULFILLED:
			{
				return state.setIn(['tags', 'fetching'], false).setIn(['tags', 'suggested'], action.payload);
			}
		...
		case _ProjectConstants.ON_PROJECT_FORM_FIELD_CHANGE:
			{
				var _action$payload = action.payload,
				    field = _action$payload.field,
				    value = _action$payload.value;

				var _nextState12 = state.setIn(['form', 'fields', field], value).setIn(['form', 'error'], null);
		...
		case _ProjectConstants.NEW_PROJECT_IMAGE_REMOVED:
			{
				...
				if (index > -1) _photos.splice(index, 1);
		...
		case _ProjectConstants.CHECK_PROJECT_VALIDATION:
			{
				var _nextState15 = state.setIn(['form', 'error'], null).setIn(['form', 'isValid'], true);
		...
		case _ProjectConstants.SAVE_PROJECT_FAILED:
			{
				return state.set('fetching', false).set('error', action);
			}

		case _ProjectConstants.SAVE_PROJECT_FULFILLED:
			{
				return state.set('fetching', false).set('fetched', true).set('error', null);
			}
	}

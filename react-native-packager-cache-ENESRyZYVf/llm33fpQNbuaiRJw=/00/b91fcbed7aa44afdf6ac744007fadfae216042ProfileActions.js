...
exports.getMyBookmark = getMyBookmark;
exports.projectTypeChanged = projectTypeChanged;
...
var _ProfileConstants = require("./ProfileConstants");
...
function getMyBookmark() {
	return function (dispatch) {
		dispatch({ type: _ProfileConstants.GET_MY_BOOKMARK });

		BackendFactory().getMyBookmark({
			user_id: "59857b51ad1e9b0c881d0c3e"
		}).then(function (res) {
			dispatch({ type: _ProfileConstants.GET_MY_BOOKMARK_FULFILLED, payload: res.data.bookmarks });
...
function projectTypeChanged(type) {
	return function (dispatch) {
		dispatch({ type: _ProfileConstants.FETCHING });

		dispatch({ type: _ProfileConstants.PROJECT_TYPE_CHANGED, payload: type });
	};
}
